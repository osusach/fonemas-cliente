export function Menu({ isMenuOpen, handleMenuOpen, isFonemasOpen, setIsFonemasOpen, isEjerciciosOpen, setIsEjerciciosOpen }) {
    ...
    return (
        <>
            ...
            <menu
                className={`fixed top-0 left-0 w-full bg-[#6610f2] text-white z-20 transform transition-transform duration-300 ${isMenuOpen ? "translate-y-0 animate-slideDown mt-16" : "-translate-y-full animate-slideUp"
                    }`}
            >
                <nav className="flex flex-col justify-center text-right w-[85%] mx-auto p-4">
                    ...
                    <a href="/extras" className="text-white text-bold text-xl my-4">Extras</a>
                </nav>
            </menu>
        </>
    );
